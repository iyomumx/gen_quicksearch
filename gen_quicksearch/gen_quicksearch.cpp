...
int init()
{
	...
	oldWndProc = reinterpret_cast<WNDPROC>(((fUnicode) ?
        SetWindowLongPtrW(plugin.hwndParent, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc)) :
        SetWindowLongPtrA(plugin.hwndParent, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc))));
	//Hotkey注册
	hotkey = { 0 };
	...
	hotkey.uMsg = static_cast<UINT>(
                    SendMessage(plugin.hwndParent,
                                WM_WA_IPC,
                                reinterpret_cast<WPARAM>( hotkey.id ),
                                IPC_REGISTER_WINAMP_IPCMESSAGE));
	genhotkeys_add_ipc = SendMessage(
                            plugin.hwndParent,
                            WM_WA_IPC,
                            reinterpret_cast<WPARAM>( &"GenHotkeysAdd" ),
                            IPC_REGISTER_WINAMP_IPCMESSAGE);
    PostMessage(plugin.hwndParent, WM_WA_IPC, reinterpret_cast<WPARAM>( &hotkey ), genhotkeys_add_ipc);
